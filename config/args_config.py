...
def get_args_parser():
    ...
    parser.add_argument('--ckpt', type=str, default='./checkpoint/', help='location of ckpt file')
    ...
    # optimization
    parser.add_argument('--num_workers', type=int, default=8, help='data loader num workers') # use, 数据加载的并行度(同时开几个子进程帮你把硬盘里的原始数据→张量→拼 batch；)
    ...
    parser.add_argument('--learning_rate', type=float, default=0.0001, help='optimizer learning rate') # use 初始学习率
    parser.add_argument('--des', type=str, default='test', help='exp description')
    parser.add_argument('--loss', type=str, default='MSE', help='loss function')
    parser.add_argument('--lradj', type=str, default='type1', help='adjust learning rate')
    parser.add_argument('--use_amp', action='store_true', help='use automatic mixed precision training', default=False)
    # 增加
    parser.add_argument("--es_patience", type=int, default=100, help="quit if no improvement after this many iterations")  # 早停耐心轮数（验证集指标 100 轮无提升就停，可省算力）
    ...
    parser.add_argument('--loss_alpha', type=float, default=0.4) # use  用于损失加权求和
    ...
    parser.add_argument('--sample_rate', type=float, default=1)  # 数据采样率
