...
# 一次性传入16000条
def traj_to_text_traj(batch_road_id, batch_time_id, batch_taxi_id, batch_dis):
    # print("开始轨迹文本化")
    # 遍历每条轨迹
...
def build_and_cache_traj_tokens(
    ...
    cached_data = torch.load(cached_pth, weights_only=True)
    ...
    for start in tqdm(range(0, num_traj, TRAJ_CHUNK), desc="Offline Tokenizing"):
        ...
        if is_class_pred:
            ...
            traj_text_list = traj_to_text_traj(
                traj_road_index_lists[start:end],
                traj_time_stamp_lists[start:end],
                traj_taxi_id[start:end],
                traj_distance_lists[start:end]
